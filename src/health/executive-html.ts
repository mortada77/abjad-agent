...
var renderer,scene,camera,head,halo,hud,eyes,clock,particleMat,particleCloud,starField,cosmicBands,avatarRoot,morphMeshes=[];
...
function initHead(){if(typeof THREE==="undefined"||!document.createElement("canvas").getContext("webgl")){document.getElementById("stateLbl").textContent="WEBGL FALLBACK";return;}
 ...
 /* Deep-space star volume: gives the entity real depth instead of a flat dotted mask. */
 var SN=innerWidth<700?1500:3000,sp=new Float32Array(SN*3),ss=new Float32Array(SN);
 for(var si=0;si<SN;si++){var sr=2.2+Math.pow(Math.random(),.55)*7.5,sa=Math.random()*6.283,sb=Math.acos(2*Math.random()-1);sp[si*3]=Math.sin(sb)*Math.cos(sa)*sr;sp[si*3+1]=Math.cos(sb)*sr*.64;sp[si*3+2]=Math.sin(sb)*Math.sin(sa)*sr-2.2;ss[si]=Math.random();}
 var sg=new THREE.BufferGeometry();sg.setAttribute("position",new THREE.BufferAttribute(sp,3));sg.setAttribute("aSeed",new THREE.BufferAttribute(ss,1));
 var sm=new THREE.ShaderMaterial({uniforms:{time:{value:0},audio:{value:0},pixel:{value:renderer.getPixelRatio()}},transparent:true,depthWrite:false,blending:THREE.AdditiveBlending,vertexShader:'attribute float aSeed;uniform float time,audio,pixel;varying float v;void main(){vec3 p=position;p.x+=sin(time*.13+aSeed*31.)*.025;p.y+=cos(time*.1+aSeed*19.)*.018;vec4 mv=modelViewMatrix*vec4(p,1.);gl_Position=projectionMatrix*mv;gl_PointSize=(.45+aSeed*1.8+audio*aSeed*2.)*pixel*(5./-mv.z);v=aSeed;}',fragmentShader:'varying float v;void main(){float d=length(gl_PointCoord-.5);if(d>.5)discard;float flare=smoothstep(.5,0.,d);vec3 c=mix(vec3(.02,.2,.8),vec3(.65,.96,1.),v);gl_FragColor=vec4(c,flare*(.2+v*.7));}'});starField=new THREE.Points(sg,sm);scene.add(starField);
 ...
 particleMat=new THREE.ShaderMaterial({uniforms:{time:{value:0},audio:{value:0},state:{value:0},pixel:{value:renderer.getPixelRatio()}},transparent:true,depthWrite:false,blending:THREE.AdditiveBlending,vertexShader:'attribute float aSeed,aDepth,aPart;uniform float time,audio,state,pixel;varying float v,vDepth;void main(){vec3 p=position;float shimmer=sin(time*(.7+aSeed)+aSeed*42.)*.018;float voice=audio*(aPart<.5&&p.y<-.12?.16:.075);p+=normalize(p+vec3(.001))*(shimmer+voice*sin(time*11.+aSeed*35.));if(state>1.5&&state<4.5){float a=(aPart>1.5?.22:.035)*time;mat2 m=mat2(cos(a),-sin(a),sin(a),cos(a));p.xz=m*p.xz;}vec4 mv=modelViewMatrix*vec4(p,1.);gl_Position=projectionMatrix*mv;float twinkle=.55+.55*sin(time*2.7+aSeed*80.);gl_PointSize=(.65+aSeed*2.45+twinkle*.55+audio*2.1)*pixel*(4.7/-mv.z);v=aSeed;vDepth=aDepth;}',fragmentShader:'varying float v,vDepth;uniform float state;void main(){vec2 q=gl_PointCoord-.5;float d=length(q);if(d>.5)discard;float core=smoothstep(.18,0.,d),glow=smoothstep(.5,0.,d);vec3 c=mix(vec3(.015,.24,.92),vec3(.72,.98,1.),v*.62+vDepth*.38);if(state>5.5)c*=.4;gl_FragColor=vec4(c,core*.82+glow*(.12+v*.3));}'});particleCloud=new THREE.Points(geo,particleMat);head.add(particleCloud);
 ...
 /* Concentric voice rings: they travel through the cosmic face with speech. */
 cosmicBands=new THREE.Group();head.add(cosmicBands);for(var bi=0;bi<7;bi++){var bg=new THREE.RingGeometry(.43+bi*.18,.438+bi*.18,96);var bm=new THREE.MeshBasicMaterial({color:bi%2?0x168cff:0x62e8ff,transparent:true,opacity:.035,side:THREE.DoubleSide,blending:THREE.AdditiveBlending,depthWrite:false});var band=new THREE.Mesh(bg,bm);band.position.set(0,-.13,.72-bi*.035);band.scale.y=.22;band.userData.phase=bi/7;cosmicBands.add(band);}
...
function animate(now){requestAnimationFrame(animate);if(!renderer)return;var t=clock.getElapsedTime();
 ...
 if(starField){starField.material.uniforms.time.value=t;starField.material.uniforms.audio.value=amp;starField.rotation.y=t*.008;starField.rotation.z=Math.sin(t*.08)*.01;}
 if(cosmicBands)cosmicBands.children.forEach(function(b,k){var pulse=(Math.sin(t*7-k*.82)+1)*.5,voice=Math.min(1,amp*1.5);var scale=1+voice*(.16+pulse*.2);b.scale.x=scale;b.scale.y=.2+voice*(.12+pulse*.12);b.material.opacity=.018+(STATE==="speaking"||STATE==="listening"?voice*(.13+pulse*.11):STATE==="thinking"?.035+pulse*.025:.012);b.rotation.z=Math.sin(t*.35+k)*.035;});
